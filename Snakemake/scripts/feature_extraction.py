    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    p4 = fd.iloc[
                        fd.index.get_loc(p3) + 1 : fd.index.get_loc(p5)
                    ].idxmax()
                ...
                try:
                    p6 = fd.iloc[
                        fd.index.get_loc(p5) + 1 : fd.index.get_loc(p5) + 10
                    ].idxmin()
                except:
                    print("error in p6")
                    pass
...
def calculate_components(
    row,
    data: pd.Series,
    idx_start_iloc="start_iloc",
    idx_end_iloc="end_iloc",
    idx_fd="fd",
    idx_fd_crossings="fd_crossings",
):
...
def calculate_features_simple(
    ...
    first = np.interp(
        -half_height, (-1) * first_half.values, np.array(first_half.index)
    )
...
def draw_featurebox(feature_df, ax, feature, by, colors, vert=False):
    d = feature_df.boxplot(
        column=feature, ax=ax, by=by, vert=vert, patch_artist=True, return_type="dict"
    )
...
components = (
    ap_derivatives.join(ap_window_iloc)
    .progress_apply(
        calculate_components, args=(raw_data,), axis=1, result_type="expand"
    )
...
features_ss_spdf = (
    components.join(ap_derivatives)
    .join(ap_window_iloc)
...
features_simple = (
    ...
    .join(ap_window_iloc)
    .progress_apply(
        calculate_features_simple, args=(raw_data,), axis=1, result_type="expand"
    )
    .rename(columns={i: f"{features_simple[i]}" for i in features_simple})
)


spikes_raw = ap_derivatives.join(spikes[["track"]])
features_raw = pd.DataFrame(
    spikes_raw["raw"].to_list(), columns=np.arange(0, len(spikes_raw["raw"].iloc[0]))
)
...
df_lengths = {"DataFrame": dataframe_list, "Length": length_list}
lengths_df = pd.DataFrame(df_lengths)

# plot boxplots of feature distribution
features_simple_fig = features_simple.join(spikes[["track"]]).pipe(
    feature_boxplots, track_colors
)

features_ss_spdf_fig = features_ss_spdf.join(spikes[["track"]]).pipe(
    feature_boxplots, track_colors
)
