...
def pad_signal(raw_data):
    ...
    filled_time_index = np.round(
        start_time + np.arange(num_samples) * sampling_interval, 6
    )
...
def create_neo_block(neo_block, raw_data, bristol_flag):
    neo_block_new = neo.Block(
        "block",
        description="Data stored in integers to avoid floating point percision problems. This works, because Dapsys seems to export the data in integers to begin with",
    )
    ...
    neo_segment_new.analogsignals.append(
        neo.AnalogSignal(
            raw_data_filled.values,
            units=pq.V,
            t_start=raw_data_filled.index[0] * pq.s,
            file_origin="data",
