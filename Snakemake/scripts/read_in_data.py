...
def get_continuous_recording(file: File, path_dapsys) -> pd.Series:
    ...
    path = path_dapsys
    total_datapoint_count = sum(
        len(wp.values) for wp in file.get_data(path, stype=StreamType.Waveform)
    )
    ...
    for wp in file.get_data(path, stype=StreamType.Waveform):
        ...
        if wp.is_irregular:
            ...
            _kernel_offset_assign(
                timestamps, wp.timestamps[0], wp.interval, current_pos, n
            )
        current_pos += n
    print("finished loading continuous recording")
    return pd.Series(
        data=values,
        index=pd.Index(data=timestamps, copy=False, name="raw_ts"),
        name="raw_amplitude",
        copy=False,
    )
...
def spike_train_to_pd_series(train: neo.SpikeTrain):
    return pd.Series(train.name, index=train.as_array().flatten(), name="track").pipe(
        rename_index, "spike_ts"
    )


# read in comments
def comments_to_pd_series(event: neo.Event):
    return pd.Series(event.labels, index=event, name="text").pipe(
        rename_index, "comment_ts"
    )
...
def get_neo_as_pd_series(neo_data, name="raw_amplitude", index_name="raw_ts"):
    return pd.Series(
        neo_data.as_array().flatten(), index=np.asarray(neo_data.times), name=name
...
if not snakemake.params.use_bristol_processing:
    ...
    with open(RECORDING_FILE, "rb") as file:
        ...
        neo_block = NIPulseStimRecordingConverter(
            file, grouping_tolerance=1e-9
        ).to_neo()

    # create Series with raw_data
    raw_data = get_continuous_recording(file, PATH_DAPSYS)
    # create df with stimulation times
    stimulations = pd.Series(
        neo_block.segments[0].events[0].as_array().flatten(), name="stimulation_ts"
    ).pipe(rename_index, "stimulation_idx")

    # create df with spike times
    spikes = (
        pd.concat(
            spike_train_to_pd_series(train)
    ...
    if snakemake.params.name == "B1":
        ...
        raw_data = -pd.Series(
            flatten_ndarray(bristol_mat["data"]),
            index=flatten_ndarray(bristol_mat["ts"]),
            name="raw_amplitude",
    ...
    stimulations = pd.Series(
        neo_segment.events[0].as_array().flatten(), name="stimulation_ts"
    ).pipe(rename_index, "stimulation_idx")
    ...
    # create df with spike times
    spikes = (
        pd.concat(spike_train_to_pd_series(train) for train in neo_segment.spiketrains)
        .sort_index()
        .to_frame()
...
spikes = (
    spikes[(spikes["spike_ts"] >= time1) & (spikes["spike_ts"] <= time2)]
...
if len(snakemake.params.tracks_to_ignore) > 0:
    spikes = (
        spikes[~spikes["track"].isin(snakemake.params.tracks_to_ignore)]
        .reset_index(drop=True)
        .pipe(rename_index, "spike_idx")
    )
track_names = sorted(spikes["track"].unique())
...
stimulations = (
    stimulations[(stimulations >= time1) & (stimulations <= time2)]
    .reset_index(drop=True)
    .pipe(rename_index, "stimulation_idx")
)

# use neo to create nix file
neo_block_new = create_neo_block(
    neo_block, raw_data, snakemake.params.use_bristol_processing
)
