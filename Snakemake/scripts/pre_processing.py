...
def ts_to_idx_col_fd_min(column, data: pd.Series, lower, upper):
    ...
    for i in range(len(index_array)):
        ...
        if not snakemake.params.use_bristol_processing:
            ...
            upsampling_factor = 2
            data_piece_upsampled = pd.Series(
                resample(data_piece, upsampling_factor * len(data_piece))
            )
...
def zerocrossings(series: pd.Series) -> pd.Series:
    ...
    return pd.Series(
        (signs[i] != signs[i - 1] for i in range(1, len(series))),
...
ap_window_iloc = (
    spikes[["spike_ts"]]
    .progress_apply(
        ts_to_idx_col_fd_min, args=(raw_data,), lower=lower, upper=upper, axis=0
    )
...
ap_derivatives = (
    ap_window_iloc[["start_iloc", "end_iloc"]]
