...
def find_threshold_bounds(ap_track_window):
    ...
    for track in ap_track_window["track"].unique():
        ...
        ap_raw = ap_track_window_sorted[["start_iloc", "end_iloc"]].progress_apply(
            extract_raw_values, args=(raw_data,), axis=1, result_type="expand"
        )
        template = ap_raw.mean()
        thresholds_template[track] = [round(min(template) - (min(template) * 0.3), 2)]
        thresholds_template[track].append(
            round(min(template) + (min(template) * 0.3), 2)
        )
...
def extract_raw_values(
    row, data: pd.Series, idx_window_start_iloc=0, idx_window_end_iloc=1
):
...
def filter_by_threshold(row, thresholds):
    ...
    min_raw = min(raw)
    return (
        round(min_raw, 2) <= thresholds[row.track][0]
        and round(min_raw, 2) >= thresholds[row.track][1]
    )
...
def plot_template(row, ax1, track_colors):
    template = pd.Series(
        row["template"],
        index=[
            round(x, 4) for x in np.arange(0, len(row["template"]) * 0.0001, 0.0001)
        ],
    )
    ax1.plot(
        [
            round(x, 4)
            for x in np.arange(0.0001, (len(template) * 0.0001 + 0.0001), 0.0001)
        ],
        template.values,
        c=track_colors[row.track],
        alpha=1,
        linestyle="dashed",
    ...
    ax1.set_ylabel("${\mu}V$", fontsize=20)
...
def plot_ap_shape(
    ...
    if type == "single":
        ...
        for track in df["track"].unique():
            ...
            for row, r in df[(df["track"] == track)][
                ["start_iloc", "end_iloc"]
            ].iterrows():
                ax.set_title(row)
                ...
                if not snakemake.params.use_bristol_processing:
                    plot_func(
                        ax,
                        np.arange(0, 3, 0.1),
                        raw_data.iloc[r[0] : r[1]].to_numpy(),
                        color=color_dict[track],
                        alpha=0.4,
                        **label_args,
                    )  # , alpha=0.2
                else:
            ...
            if templates is not None:
                ax.plot(
                    templates[track].to_numpy(), color=accent_color, label="template"
                )
            if template2 is not None:
                ...
                if not snakemake.params.use_bristol_processing:
                    ax.plot(
                        np.arange(0, 3, 0.1),
                        template_track["template"].values[0],
                        color="blue",
                        label=(
                            "Template computed"
                            if "Template computed" not in legend_labels
                            else ""
                        ),
                    )
                else:
...
ap_track_window["drop_min"] = ap_track_window.join(ap_derivatives).progress_apply(
    filter_action_potentials, args=(ap_templates,), axis=1, result_type="expand"
)
...
ap_templates.progress_apply(
    plot_template, args=(ax1, track_colors), axis=1, result_type="expand"
)
fig.tight_layout()

# plot raw waveforms
templates = None
ap_window_iloc.join(spikes[["track"]]).pipe(
    plot_ap_shape,
    raw_data,
    templates=templates,
    template2=ap_templates,
    style="plot",
)
