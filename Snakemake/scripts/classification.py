...
def compute_scores_classification(true_label, predicted_label, feature_name):
    ...
    for func in score_funcs_classification:
        if func[0] == "Accuracy":
            classification_score[func[0]] = call_score_func(
                func[1], [true_label, predicted_label]
            )
        # for other functions each class has its own score, average it
        else:
            classification_score[func[0]] = call_average_func(
                func[1], [true_label, predicted_label]
            )
...
def classification_PCA(
    df_features,
    df_features_notracks,
    color_palette,
    dataset_name,
    key,
    feature_name,
    ...
    #     y="PC2",
    #     s=100,
    #     hue=df_features["track"].to_list(),
    #     palette=color_palette,
    #     data=pca_df,
    # ).set(title=f"PCA of {key} - {dataset_name}")
...
features_tracks_basic = features_simple.join(spikes[["track"]])
key = "simple features"
...
features_ss_spdf_fv3_tracks = features_ss_spdf_fv3.join(spikes[["track"]])

features_ss_spdf_tracks = features_ss_spdf.join(spikes[["track"]])
...
features_raw_tracks = features_raw.join(spikes[["track"]])
...
classification_PCA(
    ...
    track_colors,
    dataset_name,
    key="PCA features of raw signal",
    feature_name="FS4",
)
classification_PCA(
    features_raw_tracks,
