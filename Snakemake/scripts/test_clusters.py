...
for d in [3]:  ##3,4,5
    ...
    if dim == 2:
        ...
        pca_df = pd.DataFrame(
            data=pca_features, columns=["PC1", "PC2", "PC3", "PC4", "PC5"]
        )
    le = LabelEncoder()
    ...
    ax.scatter(
        ...
        pca_df["PC3"],
        s=10,
        c=[track_colors[track] for track in spikes["track"].to_list()],
        marker="o",
        alpha=0.4,
    )
    ax.set_xlabel("PC1")
    ...
    # clustering
    kmeans_pca = KMeans(
        n_clusters=n_cluster, init="k-means++", n_init="auto", random_state=5
    )
    ...
    ax.scatter(
        ...
        s=10,
        c=[
            track_colors[track] for track in list(le.inverse_transform(predicted_label))
        ],
        marker="o",
        cmap=color_palette,
        alpha=0.4,
